  ...
  module ActsAsAuthentic
    ...
    module Login
      ...
      module Config
        ...
        def require_login_field(value = nil)
          rw_config(:require_login_field, value, true)
        end
        alias_method :require_login_field=, :require_login_field
      ...
      module Methods
        ...
        def self.included(klass)
          klass.class_eval do
            ...
            if validate_login_field && login_field
              validates_length_of login_field, validates_length_of_login_field_options.merge({:if => :require_login_field? })
              validates_format_of login_field, validates_format_of_login_field_options.merge({:if => :require_login_field? })
              validates_uniqueness_of login_field, validates_uniqueness_of_login_field_options.merge({:if => :require_login_field? })
            end
          end
        end
        ...
        module InstanceMethods
          private
          def require_login_field?
            self.class.require_login_field
          end
        end
      end
    end
