...
var TextileEditor = function()
{
	...
	// create the toolbar
	self.initialize = function(canvas, view)
	{
		...
		self.canvas = canvas;
		self.canvas.parentNode.insertBefore(toolbar, self.canvas);
		self.openTags = new Array();
		
		// Create the local Button array by assigning theButtons array to edButtons
		var edButtons = new Array();
		edButtons = self.buttons;
		
		var standardButtons = new Array();
		
		for (var i = 0; i < edButtons.length; i++)
		{
			var thisButton = self.prepareButton(edButtons[i]);
	...
	self.prepareButton = function(button)
	{
		...
		if (button.standard)
		{
			...
			img.src = TEH_PATH + 'images/textile-editor/' + button.display;
	...
	// (edAddTag)
	self.addTag = function(button)
	{
		if (button.tagEnd != '')
		{
			self.openTags[self.openTags.length] = button;
	...
	// (edRemoveTag)
	self.removeTag = function(button)
	{
		for (i = 0; i < self.openTags.length; i++)
		{
			if (self.openTags[i] == button)
			{
				self.openTags.splice(button, 1);
				// var el = document.getElementById(button.id);
	...
	// (edCheckOpenTags)
	self.checkOpenTags = function(button)
	{
		var tag = 0;
		for (i = 0; i < self.openTags.length; i++)
		{
	...
	// (edInsertTag)
	self.insertTag = function(button, tagStart, tagEnd)
	{
		...
		if (textSelected) 
		{
			...
			else // swap in and out the simple tags around a selection like bold
			{
		...
		else // just swap in and out single values, e.g. someone clicks b they'll get a *
		{
			var buttonStart = '';
			var buttonEnd = '';
			...
			if (!self.checkOpenTags(button) || button.tagEnd == '') // opening tag
			{
...
} // end class TextileEditor

var TextileEditor = new TextileEditor();	// initialize TextileEditor

// Add buttons
document.write('<script src="' + TEH_PATH + 'javascripts/textile-editor-config.js" type="text/javascript"></script>');

// Add the jQuery plugin
jQuery.fn.TextileEditor = function(p_type)
{
	return this.each(function() {
		TextileEditor.initialize(this, p_type);
	});
}